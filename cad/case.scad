...
// Bottom = tray (floor + walls). Top = lid with underside bosses, already
// flipped for print (visible face on the bed).
...
module rounded_rect(w, h, r) {
    ...
    else offset(r=r) offset(r=-r) square([w, h], center=false);
}
...
module post_sites() {
    xs = [0, C_COLS*C_PITCH];
    for (x = xs) {
        at_row(0) translate([x, 0, 0]) children();
        at_row(C_ROWS-1) translate([x, C_PITCH, 0]) children();
        if (C_ROWS >= 4)
            for (r = [2:2:C_ROWS-2])
    ...
    if (C_COLS >= 4)
        for (c = [2:2:C_COLS-2]) {
            at_row(0) translate([c*C_PITCH, 0, 0]) children();
            at_row(C_ROWS-1) translate([c*C_PITCH, C_PITCH, 0]) children();
        }
}
...
module row_outer(i) {
    at_row(i)
        translate([-C_WALL, row_y0(i), 0])
            cube([case_w(), row_ylen(i), C_BOT + C_INNER + C_REBATE]);
}
...
module m3_cs_hole() {
    translate([0,0,-0.1]) cylinder(d=C_M3, h=C_BOT+C_INNER+1);
...
module bottom_tray() {
    difference() {
        union() {
            for (i = [0:C_ROWS-1]) row_outer(i);
            post_sites()
                translate([0,0,C_BOT]) cylinder(d=C_POST, h=C_INNER);
        }
        for (i = [0:C_ROWS-1]) row_cavity(i);
        post_sites() m3_cs_hole();
        if (C_NWALL > 0)
            for (i = [0:C_NWALL-1])
...
module top_lid_use() {
    ...
    difference() {
        union() {
            for (i = [0:C_ROWS-1])
                at_row(i) translate([0.4, 0, z0])
                    cube([C_COLS*C_PITCH-0.8, C_PITCH, C_TOP]);
            post_sites()
                translate([0,0,z0]) rotate([180,0,0]) difference() {
                    cylinder(d=C_POST-0.4, h=C_INNER-0.2);
